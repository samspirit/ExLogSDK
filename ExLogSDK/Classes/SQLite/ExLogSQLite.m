...
@implementation ExLogSQLite

- (instancetype)init
{
    self = [super init];
    if (self) {
        NSString *fileName = [NSString stringWithFormat:@"ExLogFile_%@.db", [NSBundle.mainBundle.infoDictionary objectForKey:@"CFBundleIdentifier"]];
        // doucment
...
{
    if (_filePath == nil) {
        NSString *fileName = [NSString stringWithFormat:@"%@_logFile.db", [NSBundle.mainBundle.infoDictionary objectForKey:@"CFBundleIdentifier"]];
        // doucment
//        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask,YES);
//        NSString *filePath = [paths objectAtIndex:0];
        // 缓存
        ...
        if (self.openSQLite) {
            ...
            if (execStatus == SQLITE_OK) {
                while (sqlite3_step(statement) == SQLITE_ROW) {
                    ...
                    NSString *logType = [[NSString alloc] initWithUTF8String:key];
                    NSString *logText = [[NSString alloc] initWithUTF8String:text];
                    //
                    NSDictionary *dict = @{@"logTime":logTime, @"logType":logType, @"logText":logText};
